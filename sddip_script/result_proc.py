...
if __name__ == "__main__":
    ...
    收集指定数量的cuts和x
    """
    train_data_path = pathlib.Path(r"D:\tools\workspace_pycharm\sddip-SCUC-6-24\sddip_lag_data_gen_24_CV\train_data")
    ...
    for i in range(5000):
        ...
        if pathlib.Path.exists(train_data_path / f"cut_processed_pkl_{num_cuts}" / f"{i+1}_cuts.pkl"):
            raise Exception(f"file already exists: {train_data_path / f'cut_processed_pkl_{num_cuts}' / f'{i+1}_cuts.pkl'}")

        cut_df_processed.to_pickle(train_data_path / f"cut_processed_pkl_{num_cuts}" / f"{i+1}_cuts.pkl")
        cut_df_processed.to_csv(train_data_path / f"cut_processed_csv_{num_cuts}" / f"{i+1}_cuts.csv", index=False)
        x_df_preocessed.to_pickle(train_data_path / f"cut_processed_pkl_{num_cuts}" / f"{i+1}_x.pkl")
        x_df_preocessed.to_csv(train_data_path / f"cut_processed_csv_{num_cuts}" / f"{i+1}_x.csv", index=False)
