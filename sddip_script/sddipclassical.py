...
from . import scenarios, ucmodelclassical, parameters, outermodel
...
class Algorithm:
    def __init__(
        ...
        self.relative_tolerance = 1e-5  # 相对误差
    ...
    def init(self,scenario_dir, scenario_df=None):  # 前向推理时直接提供scenario_df
        ...
        self.n_samples_primary = 3  # 前向过程采样的场景路径数
    ...
    def run(self, file_name):
        ...
        lag_count = 0
        while lag_count < 20:
            v_lower, obj_mean, v_upper_l, v_upper_r = self.lag_iteration(i, CL_flag=False)
            obj_list.append(obj_mean)
            LB_list.append(v_lower)
            UB_list.append(v_upper_l)
            if len(LB_list) >= stop_stabilization_count + 1 and \
    ...
    def run_time(self, file_name, result_path=None, return_cuts=False):
        self.logger.info("#### SDDiP-Algorithm started ####")
        self.n_samples = self.n_samples_primary
        stop_stabilization_count = 3
        ...
        if result_path is not None:
            ...
            plt.legend()
            plt.grid(True)
            lb_fig_path = os.path.join(result_path,"figs", f"{file_name}_LB.png")
            plt.savefig(lb_fig_path)
            plt.close()
            ...
            plt.legend()
            plt.grid(True)
            obj_fig_path = os.path.join(result_path, "figs", f"{file_name}_obj.png")
            plt.savefig(obj_fig_path)
            plt.close()

            np.savetxt(os.path.join(result_path, "list", f"{file_name}_LB.txt"), LB_list)
            np.savetxt(os.path.join(result_path, "list", f"{file_name}_obj.txt"), obj_list)

        if return_cuts:
            cuts_tensor = self.get_all_cuts_tensor(n_iterations=i)
            return run_time, obj_list[-1], cuts_tensor
        else:
            return run_time, obj_list[-1]
    ...
    def run_n_lag(self, max_lag):
        ...
        for i in range(max_lag):
            v_lower, obj_mean, v_upper_l, v_upper_r = self.lag_iteration(i, CL_flag=False)  # iteration中cut从i=1开始
            obj_list.append(obj_mean)
    ...
    def run_sddip_statistical(self, index, n_samples_statistical):
        ...
        LB_UB_path = os.path.join(self.train_data_path, "LB_UB", f"{index}_LB_UB.csv")
        import csv
        # 写入 CSV 文件
    ...
    def SB_iteration(self, i):
        ...
        # Forward pass i来源于backward，前向i = 后向i-1
        v_opt_k = self.forward_pass(i, samples)  # 覆盖
    ...
    def lag_iteration(self, i, CL_flag):

        n_samples = self.n_samples
        samples = self.sc_sampler.generate_samples(n_samples)  # samples：[[1,2,5,3][]] len(samples)代表前向步骤执行次数（采样次数）,每个其中数字代表realization序号
            ...
            for k in range(n_samples):
                ...
                for n in range(n_realizations):
                    ...
                    if pi0_star < 1e-6 or not flag:
                        continue
                    # print("level_bundle_methods flag: true")
    ...
    def backward_benders(self, iteration: int, samples: list):  # SB
        self.cut_add_flag = True
        i = iteration
        n_samples = len(samples)
        for t in reversed(range(1, self.problem_params.n_stages)):
            for k in range(n_samples):  # nums of trial points
                ...
                x_bs_trial_point = stage_result[2]
                soc_trial_point = stage_result[3]
                theta_trial_point = stage_result[4]
                # print("x_trial_point: ", np.array(x_trial_point).shape)
                # print("y_trial_point: ", np.array(y_trial_point).shape)
                # print("x_bs_trial_point: ", np.array(x_bs_trial_point).shape)
                # print("soc_trial_point: ", np.array(soc_trial_point).shape)
                trial_point = (
                        x_trial_point
                        + y_trial_point
                        + [val for bs in x_bs_trial_point for val in bs]
                ...
                for n in range(n_realizations):
                    # Create forward model  lp
                    uc_fw_benders = ucmodelclassical.ClassicalModel(
                        self.problem_params.n_buses,
                        self.problem_params.n_lines,
                        self.problem_params.n_gens,
                        self.problem_params.n_storages,
                    ...
                    uc_fw_benders: ucmodelclassical.ClassicalModel = (
                        # 问题约束中没有Xa(n),转换为z与Xn的约束
                        self.add_problem_constraints(uc_fw_benders, t, n)
                    )
                    uc_fw_benders.add_objective(self.problem_params.cost_coeffs)
                    # print("self.problem_params.cost_coeffs")
                    # print(self.problem_params.cost_coeffs)
                    # return
                    '''
                    下边就是计算z与trial_point的差值relaxed_terms，前向过程需要relaxed_terms==0
                    '''
                    uc_fw_benders.calculate_relaxed_terms(
                        x_trial_point,
                        y_trial_point,
                        x_bs_trial_point,
                        soc_trial_point,
                    ...
                    # 置0
                    uc_fw_benders.zero_relaxed_terms()

                    # uc_fw_benders.model.write("benders_model.lp")
                    uc_fw_benders.model.optimize()
                    copy_constrs = uc_fw_benders.sddip_copy_constrs
                    ...
                    try:
                        ...
                        uc_fw_benders.model.write("model.lp")
                        uc_fw_benders.model.computeIIS()
                        uc_fw_benders.model.write("model.ilp")
                        raise
                ...
                v = np.mean(opt_values)
                pi = np.average(dual_multipliers, axis=0)
                intercept = v - pi @ np.array(trial_point)
                self.cuts_storage.add(i, k, t - 1, pi.tolist() + [intercept])
                self.x_storage.add(i, k, t - 1, trial_point)
                self.logger.info(f"benders_pi_intercept: {pi.tolist() + [intercept]}")
    ...
    def level_bundle_methods(
        ...
        while iter < iteration_limit:
            ...
            # 更新pi_hat, pi0_hat
            inner_model.add_inner_objective(self.problem_params.cost_coeffs, pi_hat, pi0_hat)
            inner_model.model.optimize()
    ...
    def lower_bound(self, iteration: int) -> float:
        ...
        # self.logger.info(f'lower bound calculating cuts num: {len(self.cuts_storage.get_values_by_t(0))}')
        # for t in range(self.problem_params.n_stages - 1):
        #     self.logger.info(f'lower bound calculating cuts num stage {t}: {len(self.cuts_storage.get_values_by_t(t))}')
    ...
    def add_problem_constraints(
        ...
        if stage < self.problem_params.n_stages - 1 and self.cut_add_flag:
            # 添加cut约束
            lag_cuts_list = self.cuts_storage.get_values_by_t(stage)
            model_builder.add_lag_cut_constrains(
                lag_cuts_list
            )
            # benders_cut_list = self.benders_cuts_storage.get_values_by_t(stage)
            # model_builder.add_benders_cut_constrains(
            #     benders_cut_list
            # )
