...
class BundleMethod(DualSolver):
    ...
    def create_subproblem(
        ...
        #     n_dual_multipliers - 1,
        #     vtype=gp.GRB.CONTINUOUS,
        #     lb=-gp.GRB.INFINITY,
        #     name="x",
        # )
        # x0 = subproblem.addVar(
        #     vtype=gp.GRB.CONTINUOUS, lb=0, name="x0"
        # )
        x = []
        for i in range(n_dual_multipliers - 1):
            x.append(
                subproblem.addVar(
                    vtype=gp.GRB.CONTINUOUS,
                    lb=-gp.GRB.INFINITY,
        ...
        x.append(subproblem.addVar(
            vtype=gp.GRB.CONTINUOUS, lb=0, name="x0"
        ))

        x_abs = subproblem.addVars(
            n_dual_multipliers - 1,
            vtype=gp.GRB.CONTINUOUS,
            lb=0,
            name="x_abs",
        ...
        for i in range(n_dual_multipliers - 1):
            subproblem.addConstr(x_abs[i] >= x[i], name=f"x_abs_{i}")
            subproblem.addConstr(x_abs[i] >= -x[i], name=f"x_abs_{i}")

        # for i in range(n_dual_multipliers):
        #     subproblem.addConstr(x_abs[i] == gp.abs_(x[i]) , name=f"x_abs_{i}")


        subproblem.addConstr(gp.quicksum(x_abs[i] for i in range(len(x_abs))) + 0.1 * x[-1] <= 1, name="L1_norm")
