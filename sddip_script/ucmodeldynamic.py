...
class ModelBuilder(ABC):
    def __init__(
        ...
        self.copy_lp_relax = copy_lp_relax
    ...
    def initialize_copy_variables(self):
        # 增加z [0,1]上下限限制
        for g in range(self.n_generators):
            if self.copy_lp_relax:  # continue
                self.z_x.append(
                    self.model.addVar(vtype=gp.GRB.CONTINUOUS, lb=0, ub=1, name="z_x_%i" % (g + 1),)
                )
                self.z_x_bs.append(
                    [
                        self.model.addVar(
                            vtype=gp.GRB.CONTINUOUS, lb=0, ub=1, name="z_x_bs_%i_%i" % (g + 1, k + 1),
                        )
                        for k in range(self.backsight_periods[g])
                    ]
                )
                ...
                self.z_x.append(
                    self.model.addVar(vtype=gp.GRB.BINARY, name="z_x_%i" % (g + 1), )
                )
                self.z_x_bs.append(
                    [
                        self.model.addVar(
                            vtype=gp.GRB.BINARY, name="z_x_bs_%i_%i" % (g + 1, k + 1),
                        )
                        for k in range(self.backsight_periods[g])
                    ]
                )
    ...
    def add_generator_constraints(self, min_generation: list, max_generation: list):
        self.model.addConstrs(
            (
                self.y[g] >= 0.1 * min_generation[g] * self.x[g] - self.delta
                for g in range(self.n_generators)
            ),
            "min-generation",
        )
        self.model.addConstrs(
            (
                self.y[g] <= 10 * max_generation[g] * self.x[g] + self.delta
                for g in range(self.n_generators)
            ),
            "max-generation",
        )
    ...
    def add_power_flow_constraints(
        ...
        self.model.addConstrs(
            (
                line_flows[l] <= 1000 * max_line_capacities[l] + self.delta
                for l in range(self.n_lines)
            ),
            "power-flow(1)",
        )
    ...
    def add_startup_shutdown_constraints(self):#s_up是决策量，约束为状态量与决策量的转换
        self.model.addConstrs(
            (
...
class BackwardModelBuilder(ModelBuilder):
    def __init__(
        ...
        self.n_x_trial = None
        self.n_y_trial = None
        self.n_x_bs_trial = None
        self.n_soc_trial = None

        self.relaxed_terms = []
        self.theta_relaxed_terms = []
    ...
    def add_relaxation(
        ...
        self.copy_vars = []
        self.n_x_trial = len(x_trial_point)
        self.n_y_trial = len(y_trial_point)
        self.n_x_bs_trial = [
            len(trial_point) for trial_point in x_bs_trial_point
        ]
        self.n_soc_trial = len(soc_trial_point)

        for j in range(self.n_x_trial):
            self.x_copy_vars.append(
                self.model.addVar(
                    vtype=gp.GRB.CONTINUOUS,
                    lb=0,
                    ub=1,
                    name="x_copy_var_%i" % (j + 1),
                )
            )

        for j in range(self.n_y_trial):
            self.y_copy_vars.append(
                self.model.addVar(
                    vtype=gp.GRB.CONTINUOUS,
                    lb=0,
                    ub=pg_max[j],
                    name="y_copy_var_%i" % (j + 1),
                )
            )

        j = 0
        for n_vars in self.n_x_bs_trial:
            self.x_bs_copy_vars.append(
                [
                    self.model.addVar(
                        vtype=gp.GRB.CONTINUOUS,
                        lb=0,
                        ub=1,
                        name="x_bs_copy_var_%i" % (k + 1),
                    )
        ...
        for j in range(self.n_soc_trial):
            self.soc_copy_vars.append(
                self.model.addVar(
                    vtype=gp.GRB.CONTINUOUS,
                    lb=0,
                    ub=soc_max[j],
    ...
    def relax(
        ...
        self.relaxed_terms = [
            x_trial_point[j] - self.x_copy_vars[j]
            for j in range(self.n_x_trial)
        ]

        self.relaxed_terms += [
            y_trial_point[j] - self.y_copy_vars[j]
            for j in range(self.n_y_trial)
        ]

        self.relaxed_terms += [
            x_bs_trial_point[g][k] - self.x_bs_copy_vars[g][k]
            for g in range(len(self.n_x_bs_trial))
            for k in range(self.n_x_bs_trial[g])
        ]

        self.relaxed_terms += [
            soc_trial_point[j] - self.soc_copy_vars[j]
            for j in range(self.n_soc_trial)
        ]
    ...
    def add_copy_constraints(
        ...
        n_y_var_approximations = len(self.y_copy_vars)
        n_soc_var_approximations = len(self.soc_copy_vars)


        self.copy_constraints_y = self.model.addConstrs(
            (
                self.z_y[i] == self.y_copy_vars[i]
        ...
        self.copy_constraints_x = self.model.addConstrs(
            (
                self.z_x[i] == self.x_copy_vars[i]
                for i in range(self.n_x_trial)
            ),
            "copy-x",
        )
        ...
        self.copy_constraints_x_bs = self.model.addConstrs(
            (
                self.z_x_bs[g][k] == self.x_bs_copy_vars[g][k]
                for g in range(len(self.n_x_bs_trial))
                for k in range(self.n_x_bs_trial[g])
            ),
            "copy-x-bs",
        ...
        self.copy_constraints_soc = self.model.addConstrs(
            (
                self.z_soc[i] == self.soc_bin_copy_vars[i]
    ...
    def check_bin_copy_vars_not_empty(self):
        if not (
            self.x_copy_vars and self.y_copy_vars and self.x_bs_copy_vars
        ):
